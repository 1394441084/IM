...
void TalkWindow::initControl()
{
	...
	switch (m_groupType)
	{
		case COMPANY:
		{
			initCompanylTalk();			// 初始化 公司群聊天
			break;
		}
		case PERSONELGROUP:
		{
			initPersonelTalk();			// 初始化 人事部聊天
			break;
		}
		case DEVELOPMENTGROUP:
		{
			initDeveloplTalk();			// 初始化 研发部聊天
			break;
		}
		case MARKETGROUP:
		{
			initMarketTalk();				// 初始化 市场部聊天
			break;
		}
		default:								// 默认设置
		{
			initPtoPTalk();					// 初始化 市场部聊天
			break;
		}
	}
	*/
...
{
	QSqlQuery queryDepID(QString("SELECT departmentID FROM qtqq.tab_department WHERE department_name = '%1'")
...
void TalkWindow::initTalkWindow()
{
	...
	*/
	...
	{	// 如果不是公司群，通过公司群号，进行过滤
		queryEmployeeModel.setQuery(QString("SELECT employeeID FROM qtqq.tab_employees WHERE status = 1 AND departmentID = %1")
			.arg(m_talkId));
	}
	...
	for (int i = 0; i < nEmployeeNum; i++)
	{
		...
		//不将自己添加进聊天项
		//if (employeeID == gLoginEmployeeID.toInt()) continue;
...
void TalkWindow::addPeopInfo(QTreeWidgetItem* pRootGroupItem, int employeeID)
{
	...
	queryInfoModel.setQuery(QString("SELECT employee_name,employee_sign,picture FROM qtqq.tab_employees WHERE employeeID = %1")
		.arg(employeeID));
...
void TalkWindow::onItemDoubleClicked(QTreeWidgetItem* item, int column)
{
	...
	if (bIsChild)
	{
		...
		if (talkId == gLoginEmployeeID.toInt()) return;
