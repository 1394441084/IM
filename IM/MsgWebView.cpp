...
void MsgHtmlObj::initHtmlTmpl()//可优化
{
	// 初始化 左侧
	// 将msgleftTmpl中的【%1】，替换成 对方头像路径
	m_msgLHtmlTmpl = getMsgtmplHtml("msgleftTmpl");
	m_msgLHtmlTmpl.replace("%1", m_msgLPicPath);
	m_msgLHtmlTmpl.replace(":", "D:/C++_source_code_Qt_VS/IT_QT/VS_QT_QQ/testQQ");

	// 初始化 右侧
	// 将msgrightTmpl中的【%1】，替换成 对方头像路径
	// gstrLoginHeadPath 是 CCMainWindow 中一个全局变量
	m_msgRHtmlTmpl = getMsgtmplHtml("msgrightTmpl");
	//QString s = "D:/C++_source_code_Qt_VS/IT_QT/VS_QT_QQ/testQQ/Resources/imagesss/E.jpeg";
	m_msgRHtmlTmpl.replace("%1", gstrLoginHeadPath);
	m_msgRHtmlTmpl.replace(":", "D:/C++_source_code_Qt_VS/IT_QT/VS_QT_QQ/testQQ");
}
	...
	, m_channel(new QWebChannel(this))
{
	...
	*/
	...
	queryEmployeeModel.setQuery(QString("SELECT departmentID FROM qtqq.tab_department WHERE department_name = '%1'")
	...
	if (strTalkID == strCompanyID)
	{
		...
		queryEmployeeModel.setQuery("SELECT employeeID,picture FROM qtqq.tab_employees WHERE status = 1");
	}
	else
	{
		// 不是公司群聊窗口
		...
		if (strTalkID.length() == 4)	// 其他群里
		{
			isGroupTalk = true;
			// 获取 某个群，里面的 群员QQ号
			queryEmployeeModel.setQuery(QString("SELECT employeeID,picture FROM qtqq.tab_employees WHERE status = 1 AND departmentID = %1")
				.arg(strTalkID));
		...
		{
			// 获取 对方QQ号
			queryEmployeeModel.setQuery(QString("SELECT picture FROM qtqq.tab_employees WHERE status = 1 AND employeeID = %1")
				.arg(strTalkID));
			// 通过索引，找出图片路径，并转成 字符串
