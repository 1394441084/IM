...
void CCMainWindow::initContacTree()
{
	...
	QString strGroupName = QString::fromLocal8Bit("假的联系项");
...
void CCMainWindow::addCompanyDeps(QTreeWidgetItem* pRootGroupItem, int DepID)
{
	...
	queryPicture.exec();
	queryPicture.first();			// 也可以用 queryPicture.next();
...
{
	// 从本地编码，转变成Unicode
	setUserName(QString::fromLocal8Bit("仿真QQ的磨难之旅"));//可优化
...
QString CCMainWindow::getHeadPicPath()
{
	...
	// 判断
	if (!m_isAccountLogin)			// QQ号登陆
	{
		QSqlQuery queryPic(QString("SELECT picture FROM qtqq.tab_employees WHERE employeeID = %1")
	...
	else		// 账号登陆
	{
		...
		QSqlQuery queryEmployeeID(QString("SELECT employeeID FROM qtqq.tab_accounts WHERE account = '%1'")
							.arg(m_account));
