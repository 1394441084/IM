...
void TalkWindowShell::getEmployeeID(QStringList& employeesList)
{
	...
	queryModel.setQuery("SELECT employeeID FROM qtqq.tab_employees WHERE status = 1");
...
/*
	...
	可优化文件接发·服务器端通过分块发送,客户端通过分块接收,能接收到超大文件
		...
		*/
		...
		if (btData[0] == '1')
		{
			...
			else if (cMsgType == '2')
			{
				...
				QSqlQuery querySenderName(QString("SELECT employee_name FROM qtqq.tab_employees WHERE employeeID = %1")
					.arg(empID));
